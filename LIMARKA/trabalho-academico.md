...
Modelos de propagação {#sec:modelos}
---------------------
...
Pela \autoref{medicao_modelos} é possível observar que, dos modelos de propagação analisados, o
...
Como a estratégia de perturbar mais de um *access point* por vez, o *Simulated Annealing* inicia
...
processo. Como são quatro parâmetros para os quais se deseja saber os valores e
são utilizados dois níveis de calibração, foi utilizado o conceito de
...
os valores foram adicionados em uma planilha do Excel elaborada pelo
...
\end{table}
 ...
 O melhor e o pior resultado dado pela configuração da \autoref{tabela_parametros_2} pode ser visto na \autoref{candidatos_parametros_2}:
...
De acordo com a configuração da \autoref{tabela_parametros_3} são dados o melhor e o pior resultado, conforme expostos na \autoref{candidatos_parametros_3}.
...
tornando-o dinâmico de acordo com o tamanho da planta. A \autoref{parametros_metaheuristica} mostra
como ficaram os parâmetros utilizados na metaheurística, calibrados através do projeto fatorial apresentado.
...
parâmetros definidos para a alocação de vários *access points* foram adaptados dos
parâmetros utilizados no *Simulated Annealing* com um *access point*.

Avaliação da solução
--------------------

Nesta seção apresenta-se como foram implementadas as técnicas
...
com que o *Simulated Annealing* irá buscar o ótimo global.
...
em torno de -85 a -100 $dBm$ (em torno de $3,162\times10^{-12} W$ a
...
considerações tais fatos, experimentou-se com diversas maneiras de
...
informações nela armazenada estivessem em $dBm$. Numa inspeção visual da
solução proposta pelo *Simulated Annealing*, com a coloração dos
valores de $dBm$ em um gradiente de cores de acordo com o posicionamento
do *access point*, pode-se observar que a partir de um ponto aleatório,
a metaheurística conduzia as soluções candidatas para a região central
do ambiente 2D, como seria esperado para um ambiente simulado sem a 
existência de paredes e obstáculos. 
...
otimização, de maneira a checar a proximidade de dados reais
...
Desde o princípio, a implementação da função objetivo para apenas
um *access point* visava maximizar a cobertura do sinal *Wi-Fi* para o ambiente.
Isso não foi diferente para a simulação de mais do que um *access point*. Foram utilizadas
...
opção válida. Com a conversão dos valores da matriz e a verificação da
qualidade do sinal adicionadas à função objetivo, o tempo de execução do algoritmo aumentou
...
mais um *access point* para buscar os seus respectivos posicionamentos ótimos, somar suas
...
ruim ou boa, é necessário transformar essa escolha em um número. Nesse
sentido, de acordo com as $n$ matrizes de resultado de $\textit{n}$
*access points*, foi necessário realizar suas sobreposições duas a duas, 
até restar apenas uma matriz de propagação representando todas as demais. 

Tal método recebe como parâmetro uma lista (ou vetor, em
...
O método que realiza essa operação pode ser visualizado a seguir.
...
Tal método retorna à matriz resultante e realiza a
avaliação da lista de *access points* para obter um valor a se
...
qualidade inferior à sensibilidade do equipamento não foram
diretamente implementados nessa estratégia, sendo utilizado 
apenas na visualização do resultado final.

Análise da utilização de recursos
---------------------------------

A fim de buscar um melhor visualização do fluxo de execução do
algoritmo, foi utilizada uma ferramenta de *profiling*, chamada *cProfile*,
...
algoritmo, de acordo com o tempo total. Como é possível ver a seguir, foi
...
Apenas com isso, o *script* Python será compilado em código de máquina no momento
que for executado (*just-in-time*). A seguir é possível ver um exemplo de
um método que foi compilado para código de máquina utilizando a
anotação *@jit*,
...
demandava mais recursos e demandava a maior parte do tempo de execução. Neste
...
cálculos. O código referente ao "simula_propagacao_gpu" pode ser visto abaixo.
...
O equipamento utilizado para o desenvolvimento deste trabalho utiliza uma placa de
vídeo NVIDIA modelo GeForce GT 740M[^25] com 2GB de memória dedicada;
possui 384 núcleos CUDA, com um *clock* básico de 993 MHz, 1.300 milhões de
...
O cálculo do valor do sinal naquele ponto tem custo de tempo $O(k)$ pois consiste em um
...
da matriz (PM) e o *access point*, podem haver $k$ paredes, para as quais deve-se verificar se
há ou não interseção entre a reta AP-PM e a reta formada pela parede.
Tal operação, utilizando a equação de geometria analítica para
...
(como o método *simulate\_kernel\_gpu*) em Python, compilá-lo e
...
Tais resultados podem ser comparados, pela \autoref{captura}
e pela \autoref{simulacao_vinicius} com os resultados previstos pela
...
pudessem ser calibrados, num processo de melhoria gradual visando ao
aumento da precisão da solução apresentada. É válido reiterar que
...
pisos e teto, desta forma, é notável que os valores estão de acordo com as medições realizadas por \cite{VINICIUS}.
...
Foi possível observar que os modelos teóricos de propagação, devidamente calibrados, 
obtiveram o comportamento relativamente parecido com a coleta
...
implementação do projeto e a explicação dos
...
ou seja, demonstrar graficamente o seu valor dentro da matriz de
...
tal resultado da simulação e dos próximos resultados que são mostrados
neste capítulo, utilizou-se a versão do *script* Python[^26] que faz uso da
GPU.
...
ambiente através do PyGame, as principais informações de configuração da execução,
bem como a qualidade do resultado obtido, são impressos textualmente no console da CLI\footnote{Interface de Linha de Comando}. 
No \autoref{saida_exemplo} é possível identificar como é invocado o *software* desenvolvido e a respectiva saída.
...
Annealing* fugiu de alguns ótimos locais e, no final da execução, retornou a melhor avaliação encontrada.
...
Neste momento, é importante ressaltar que a \autoref{captura_inicial} e a \autoref{capturas} representam os resultados
dos primeiros testes executados. O gradiente de cores utilizada foi o
"plasma", variando das cores laranja até o azul escuro, onde a cor azul representa
as zonas com menor intensidade e a cor laranja as zonas de maior intensidade do sinal Wi-Fi. Na próxima seção
são mostrados os resultados de testes efetuados utilizando ainda um
...
ajustado para as características dos edifícios do e IFMG *campus* Formiga. 
A \autoref{prop_bloco_a}, se comparada com a \autoref{captura_inicial}, possui cores mais
intuitivas para facilitar a interpretação dos resultados obtidos, além de uma restrição na parte
gráfica, destacada com a cor preta nas áreas onde o sinal é menor que a
sensibilidade típica de um equipamento *Wi-Fi* (desktop, notebook, smartphone ou tablet). 
Nesta nova escala de cores, a cor verde representa as zonas de maior intensidade do sinal e 
...
É possível notar que o *Simulated Annealing* definiu como melhor posicionamento, nesta situação, um
...
contém o maior fluxo de pessoas que utilizam o acesso à internet. 
...
com um *access point* no bloco A, com cada fatia correspondendo à faixa de qualidade do sinal propagado.
Observe que as zonas de sombra (sem cobertura do sinal) tiveram uma fatia correspondente a 
mais de um quarto de todo o espaço e, se somado com a fatia de sinal ruim, mais da metade da 
...
Como pode ser visto nas Figuras \ref{prop_bloco_a} e \ref{captura_zona}, quando utilizada a busca pelo posicionamento ótimo
para apenas um *access point*, ele tendeu a permanecer ao centro da planta. Com
o resultado dado para esta simulação com um *access point*, os cômodos mais ao
centro são bem atendidos com o sinal *Wi-Fi*. Já os cômodos mais afastados não 
irão usufruir de uma boa qualidade de sinal, esse é
...
folga, fazendo com que seu posicionamento possa aproveitar a propagação do espectro no
ambiente. Com a escolha sugerida pelo *Simulated Annealing*, utilizando ambos *access points*
transmitindo a -25 $dBm$, pode-se também notar que algumas partes das
salas nos extremos não estão recebendo uma boa qualidade de sinal como na
sala em que os *access points* estão instalados, mas, mesmo assim, a maior parte da
área simulada está recebendo algum sinal, havendo poucas zonas onde a potência recebida
...
Os valores dados na saída para a simulação da \autoref{captura_3_aps_bloco_a} com três *access points*
...
regressão logístico *(NP-Log)*. Sugere-se incluir no software algum tratamento para equipamentos de diferentes configurações, não tendo todos os access points a mesma potência de transmissão, que poderia inclusive ser calibrada pelo própria metaheurística como alguma fração da potência máxima de cada equipamento. Seria promissor se o software conseguisse testar quantidades incrementeis de access points, de maneira a identificar a quantidade mínima de equipamentos que atendesse à taxa de cobertura desejada. Também, sugere-se que seja utilizado
...
para distâncias perto como longe. Portanto, sugere-se utilizar dois modelos
de propagação, um para perto $(1-20 m)$ e outro para mais longe $(20-100
...
incluída em trabalhos posteriores. Ainda, caso haja demanda, sugere-se a
...
dependências dos blocos A e C do  IFMG *campus* Formiga; (iii) aplicar a metaheurística computacional
...
Na versão final, o *software* é capaz de receber como entrada uma representação do ambiente, realizando a simulação de propagação dos sinais considerando a atenuação nas paredes e no espaço livre. A metaheurística buscou maximizar a cobertura do sinal *Wi-Fi* nas dependências do IFMG *campus* Formiga. Assim, também foi possível construir um modelo de propagação que se adequasse às dependências do IFMG *campus* Formiga, a partir da representação do ambiente fornecida pelo engenheiro civil do *campus*. Além disso, é possível realizar a simulação da propagação do sinal *wireless* de *access points* *Wi-Fi* através das dependências do *campus*, informando ao *software* quantos *access points* se deseja alocar. Quando houver necessidade de ampliar, mesmo que temporariamente, a rede para a acomodação de novos pontos de acesso para telecomunicação, será fácil decidir os melhores locais para a alocação dos equipamentos. Com a solução desenvolvida, se torna viável ter uma rede *Wi-Fi* com boa qualidade de sinal, fazendo com que a cobertura de sinal possa abranger boa parte da área necessária, provendo uma qualidade de serviço satisfatória aos que fazem uso da rede.

Como mencionado anteriormente, a ferramenta desenvolvida foi disponibilizada de forma gratuita, utilizando Licença Pública Geral GNU — GPL no site *github.com*[^28]. Para reforçar sua relevância, ressalta-se que ela possibilita testar disposições de *access points* sem
