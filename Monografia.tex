...
%\lstinputlisting[language=TeX]{code/pseudo_codigo.py}
...
\begin{table}[]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{|c|c|c|c|c|c|}
			...
			\begin{tabular}[c]{@{}c@{}}WIDTH * 0.025\\ (2.5\% da largura da matriz)\end{tabular} & 600 * nº de APs                                                                & 5                                                                                 & 80                                                                            & 85\%                                                                                  & 300 * nº de APs                                                            \\ \hline
		\end{tabular}%
	}
\end{table}
...
As plantas dos pisos 1, 2 e 3 do bloco C foram obtidas em formato DWG e convertidas para o formato \textit{.dxf }ao final da implementação. Tais arquivos foram simulados e obtidas suas representações gráficas. A imagem abaixo mostra a simulação da planta baixa no bloco C utilizando 1 AP.
...
\begin{figure}[ht]
	...
	\begin{center}
		\includegraphics[scale=0.3]{images/captura-zona.jpg}
	\end{center}
	\legend{Fonte: Elaboração do autor}
\end{figure}
...
O comportamento da função objetivo da \autoref{captura_3_aps_bloco_c} pode ser visto com a \autoref{fo_captura_3_aps_bloco_c}.

\begin{figure}[ht]
	\caption{\label{fo_captura_3_aps_bloco_c}comportamento da função objetivo com simulação da propagação de sinais de microondas no bloco C utilizando 3 APs.
	}
	\begin{center}
		\includegraphics[scale=0.6]{images/grafico-sa-3-aps-bloco-c.jpg}
	\end{center}
	\legend{Fonte: Elaboração do autor}
\end{figure}


Com a \autoref{captura_3_aps_bloco_a} é possível ver propagação do sinal utilizando 3 access points no bloco A com equipamentos transmitindo a -25 dBm.
...
O comportamento da função objetivo da \autoref{captura_3_aps_bloco_a} pode ser visto com a \autoref{fo_captura_3_aps_bloco_a}.

\begin{figure}[ht]
	\caption{\label{fo_captura_3_aps_bloco_a}comportamento da função objetivo com simulação da propagação de sinais de microondas no bloco A utilizando 3 APs.
	}
	\begin{center}
		\includegraphics[scale=0.6]{images/comportamento-fo-3-ap-bloco-a.jpg}
	\end{center}
	\legend{Fonte: Elaboração do autor}
\end{figure}

A os valores dados na saída para a simulação da \autoref{fo_captura_3_aps_bloco_a}  com três \textit{access points} no bloco A são os seguintes:
